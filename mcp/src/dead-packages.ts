...
export function detectDeadPackages(
  ...
  // Collect all imported packages across all code files
  const usedPackages = new Set<string>();
  for (const src of codeContents) {
    for (const pkg of extractImportedPackages(src)) {
  ...
  for (const { path: pkgPath, content } of packageJsonEntries) {
    ...
    for (const [name, version] of Object.entries(devDeps)) {
      if (ALWAYS_USED.has(name)) continue;
      // @types/* packages — only dead if the base package isn't imported
      if (name.startsWith('@types/')) {
        const base = name.slice(7); // strip "@types/"
...
export async function findDeadPackages(
  ...
  return detectDeadPackages(packageJsonContents, [...codeContents.values()]);
}
