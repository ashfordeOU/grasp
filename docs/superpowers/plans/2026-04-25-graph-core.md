...
```bash
grep -n "3.10.0\|3.11.0" GraspRepo/mcp/server.json
```
...
```bash
grep -n "3.10.0" GraspRepo/gpt-actions/src/server.ts
```

For `index.html` — find both `GRASP_VERSION` occurrences:
```bash
grep -n "GRASP_VERSION\|3.10.0" GraspRepo/index.html | head -10
```
...
```bash
cd GraspRepo/browser-extension
