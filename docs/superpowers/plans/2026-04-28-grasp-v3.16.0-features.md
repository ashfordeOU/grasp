...
```bash
mkdir -p GraspRepo/.github/actions/grasp-pr-impact
```
...
```bash
grep -n "colorMode\|setColorMode\|nodeColor\|const \[show" GraspRepo/index.html | head -20
```
...
```bash
grep -n "background-color\|nodeColor\|getColor\|fill:" GraspRepo/index.html | head -15
```
...
```bash
grep -n "colorMode\|toolbar\|btn\|button.*click" GraspRepo/index.html | grep -i "button\|btn" | head -10
```
