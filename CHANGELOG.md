...
### Safari Extension
- **Mac App Store:** Grasp is now available for Safari on macOS 13+ — same floating button, popup, and custom host support as Chrome and Firefox
...
- **Zero new TS code:** all three browsers (Chrome, Firefox, Safari) share the same compiled `background.js`, `content.js`, and `popup.js` output
- **Xcode project:** `safari-extension/Grasp/` — generated by `xcrun safari-web-extension-converter`, references `browser-extension/dist-safari/` as a folder reference
...
- **CI:** `build-safari` job on `macos-latest`; archives with `xcodebuild`, exports `.app`, submits to App Store Connect via `xcrun altool` (skips gracefully if `APPLE_API_KEY_ID` secret not set)
