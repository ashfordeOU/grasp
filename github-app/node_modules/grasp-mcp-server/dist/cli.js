...
var GitHubSource = class {
  ...
  owner;
  repo;
  constructor(owner, repo, token2) {
    this.owner = owner;
    this.repo = repo;
    this.octokit = new Octokit2({ auth: token2 });
  }
  async getFileTree() {
...
async function analyzeSource(source, onProgress) {
  ...
  connections.forEach((c2) => {
    couplingMap[c2.target] = (couplingMap[c2.target] ?? 0) + 1;
  });
  ...
  const adjMap = {};
  connections.forEach((c2) => {
    if (!adjMap[c2.source]) adjMap[c2.source] = [];
    adjMap[c2.source].push(c2.target);
  });
...
function parseSource(input, token2) {
  if (isLocalPath(input)) {
    return { type: "local", path: resolveLocalPath(input) };
  }
  const gh = parseGitHubUrl(input);
  if (gh) {
    return { type: "github", owner: gh.owner, repo: gh.repo, token: token2 };
  }
  return null;
}
...
var report = flags.has("--report");
var noOpen = flags.has("--no-open");
var rulesCI = flags.has("--rules");
var watchMode = flags.has("--watch");
var timelineMode = flags.has("--timeline");
var prComment = flags.has("--pr-comment");
var port = parseInt(process.env.GRASP_PORT || "7331", 10);
...
function findIndexHtml() {
  const candidates = [
    (0, import_path2.join)(__dirname, "..", "..", "index.html"),
    (0, import_path2.join)(__dirname, "..", "index.html"),
    (0, import_path2.join)(__dirname, "index.html")
  ];
  for (const c2 of candidates) {
    if ((0, import_fs2.existsSync)(c2)) return (0, import_path2.resolve)(c2);
  }
  return null;
}
function openBrowser(url) {
  const cmd = process.platform === "darwin" ? `open "${url}"` : process.platform === "win32" ? `start "" "${url}"` : `xdg-open "${url}"`;
  (0, import_child_process2.exec)(cmd, (err) => {
...
function loadRulesFile(dir) {
  const p = (0, import_path2.join)(dir, ".grasprules");
  if (!(0, import_fs2.existsSync)(p)) return null;
  try {
    const obj = JSON.parse((0, import_fs2.readFileSync)(p, "utf-8"));
...
function printRulesReport(violations, rules) {
  console.log(c.bold("\n  \u{1F3DB}\uFE0F  Architecture Rules Check\n"));
...
function serveAndOpen(indexHtml, result, repoInfo, watchPath, timeline) {
  const raw = (0, import_fs2.readFileSync)(indexHtml, "utf8");
  const buildPreload = (r) => {
  ...
  const buildHtml = (r) => raw.replace("</head>", `${buildPreload(r)}
</head>`);
  let latestResult = result;
  const sseClients = [];
  function pushUpdate(newResult) {
...
`;
  ...
  const srv = (0, import_http.createServer)((req, res) => {
  ...
  srv.listen(port, "127.0.0.1", () => {
    ...
    console.log(c.bold(`  \u{1F52D} Grasp is ready \u2192 ${url}`));
    if (watchPath) console.log(c.green(`  \u{1F441}  Watching ${watchPath} for changes...`));
  ...
  if (watchPath) {
    ...
    try {
      (0, import_fs2.watch)(watchPath, { recursive: true }, (_evt, filename) => {
...
function openFileUrl(indexHtml, repoParam) {
  const fileUrl = `file://${indexHtml}?repo=${encodeURIComponent(repoParam)}`;
  console.log(c.bold(`  \u{1F52D} Opening Grasp \u2192 ${fileUrl}`));
  console.log("");
  if (!noOpen) openBrowser(fileUrl);
}
async function main() {
  const source = parseSource(
    target.startsWith(".") || target.startsWith("/") || target.startsWith("~") ? (0, import_path2.resolve)(target.replace(/^~/, process.env.HOME || "~")) : target,
    token
  );
  if (!source) {
  ...
  const spin = isTTY && !report ? setInterval(() => {
    process.stdout.write(`\r${spinFrames[spinIdx++ % 10]} ${lastMsg}        `);
  }, 80) : null;
  let result;
  try {
...
`);
  ...
  if (rulesCI) {
    ...
    if (!rules) {
      ...
      if (violations.length > 0) {
        const out2 = "grasp-report.json";
        try {
          (0, import_fs2.writeFileSync)(out2, JSON.stringify({ ...result, archRuleViolations: violations, ci: { passed: false, score: result.summary.healthScore, failures: violations.map((v) => v.rule + ": " + v.from + " \u2192 " + v.to) } }, null, 2));
        } catch {
        }
        process.exit(1);
      }
      ...
      try {
        (0, import_fs2.writeFileSync)(out, JSON.stringify({ ...result, archRuleViolations: [], ci: { passed: true, score: result.summary.healthScore, failures: [] } }, null, 2));
      } catch {
      }
      process.exit(0);
    }
  ...
  if (prComment) {
    ...
    const secBadge = s.securityIssueCount > 0 ? ` \u{1F510} ${s.securityIssueCount} security` : "";
    const comment = [
      "## \u{1F4CA} Grasp Health Report",
      "",
      "| Metric | Value |",
      "|--------|-------|",
  ...
  if (report) {
    const out = "grasp-report.json";
    try {
      (0, import_fs2.writeFileSync)(out, JSON.stringify(result, null, 2));
  ...
  if (source.type === "github") {
    ...
    const timeline = timelineMode && source.path ? getGitTimeline(source.path, 30) : void 0;
    if (timeline && timeline.length > 0) console.log(c.dim(`  \u{1F4C5} Timeline: ${timeline.length} commits loaded`));
    serveAndOpen(indexHtml, result, repoInfo, watchMode ? source.path : void 0, timeline);
  }
}
main().catch((err) => {
