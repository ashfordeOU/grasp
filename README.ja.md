...
**130 個の MCP ツール + 8 個のリソース + 2 個のプロンプト · 35 言語 · 11 AI プロバイダー + OpenRouter 経由で 200+ モデル · 10 種類のグラフビュー · データ収集ゼロ**
...
| カテゴリ | 追加内容 |
|----------|----------|
| **グラフ解析** | `grasp_hub_nodes`, `grasp_bridge_nodes`, `grasp_surprising_connections`, `grasp_knowledge_gaps`, `grasp_suggested_questions` — 次数中心性、Brandes 媒介中心性、稀なクロスレイヤーエッジ検出、孤立/未テストホットスポット発見、自動生成レビュー質問 |
| **LLM コンテキストツール** | `grasp_minimal_context`（サブ 100 トークンのオリエンテーション）、`grasp_traverse`（トークンバジェット BFS）、`grasp_semantic_search`（関数シグネチャ上のコサイン類似度）、`grasp_apply_refactor`（dry-run プレビュー付きリネーム実行） |
| **アーキテクチャインテリジェンス** | `grasp_architecture_overview` — コミュニティ + ハブ + 質問の統合レポート |
| **グラフエクスポート** | `grasp_export_graphml`, `grasp_export_cypher`, `grasp_export_obsidian` — yEd/Gephi GraphML、Neo4j CREATE 文、Obsidian Canvas |
| **インポートリゾルバー** | TS-config パスエイリアス解決 (`@/components` → `src/components`)、Jedi スタイルの Python 相対インポート + `__init__.py` |
| **ワークフロー** | Claude Code スラッシュコマンド (`/grasp:build-graph`, `/grasp:review-delta`, `/grasp:review-pr`)、トークン削減評価ハーネス (`scripts/eval-token-reduction.mjs`) |
| **ブラウザ UX** | お試しチップ、トークンインジケーター、スナップショット URL、2 リポジトリ比較モーダル、解析中のレート制限リカバリ、モバイルグラフタッチジェスチャー、フローティングキーボードショートカットポップオーバー、リポジトリ別永続化、拡張エクスポートメニュー |
| **i18n** | 多言語 READMEs — हिन्दी · 日本語 · 한국어 · 简体中文 |

合計: 130 個の MCP ツール（旧 121）、13 個の新ツール、10 個の新ブラウザ UX、22 個の新ユニットテスト。

---

## Grasp とは？

**Grasp** は、任意の GitHub または GitLab リポジトリ — クラウドまたはセルフホスト — あるいはローカルコードベースを数秒でインタラクティブなアーキテクチャマップに変換します。**130 個の MCP ツール**(加えて 8 個のリソースと 2 個のガイド付きプロンプト)が、解析エンジン全体を Claude Code、Cursor、および任意の MCP 互換エージェントに公開します。

```
URL を貼り付け / フォルダーを開く  →  AST 解析エンジン  →  アーキテクチャマップ + 130 MCP ツール
```
...
| **130 個の MCP ツール** | 依存関係グラフ、セキュリティ、**OSV.dev SCA 脆弱性スキャン**、DORA、Brain ストア、Kuzu graph schema v3、コミュニティ、ORM トラッカー、git 変更影響、アーキテクチャドリフト検出、テストカバレッジギャップマップ、組織ダッシュボード、PR impact action、MCP Resources/Prompts、`grasp setup` エディタ自動設定 |
...
| IDE | インストール |
|-----|---------|
| **VS Code** | [Install (.vsix)](https://github.com/ashfordeOU/grasp/releases/latest) — `grasp-vscode-3.18.0.vsix` をダウンロードし、**Extensions: Install from VSIX…** (`Cmd+Shift+P`) を実行 |
...
| AI ツール | インストール方法 | 備考 |
|---------|----------------|-------|
| **Claude Code** | `claude mcp add grasp -- npx -y grasp-mcp-server` | ネイティブ MCP — 全 130 ツール + 8 Resources + 2 Prompts |
...
> **インストール:** [GitHub Releases](https://github.com/ashfordeOU/grasp/releases/latest) から `grasp-vscode-3.18.0.vsix` をダウンロードし、VS Code で **Extensions: Install from VSIX…** (`Cmd+Shift+P`) を実行。
...
`index.html` と `team-dashboard.html` の両方がフッターに現在のバージョン (`v3.18.0`) を表示します。読み込み時に、新しいリリースを npm レジストリで静かに確認します。見つかった場合、消去可能なトーストが表示されます:
...
**130 MCP ツール · 35 言語 · 11 AI プロバイダー + 200+ モデル · インストール不要 · データ収集ゼロ**
