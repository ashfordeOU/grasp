...
Both `index.html` and `team-dashboard.html` display the current version (`v3.3.12`) in the footer. On load, they silently check the npm registry for a newer `grasp-mcp-server` release. If one is found, a dismissible toast appears:
